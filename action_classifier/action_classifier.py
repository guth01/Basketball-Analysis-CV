"""
action_classifier/action_classifier.py — Shot event detection (made/miss classification).
...
from settings.action_config import (
    ROBOFLOW_MODEL_ID,
...
class ActionClassifier:
    """
    Classifies shot events (made / missed) using class-level signals from
    the RF-DETR Roboflow model: player-jump-shot, player-layup-dunk, ball-in-basket.

    Usage:
        classifier = ActionClassifier(fps=30)
        for frame_index, frame in enumerate(video_frames):
            detections = classifier.run_inference(frame)
            events = classifier.update(frame_index, detections)
        classifier.save_events(Path("output/shot_events.json"))
    """
    ...
    def __init__(self, fps: int = 30):
        ...
        print(f"[ActionClassifier] Loading Roboflow model: {ROBOFLOW_MODEL_ID}")
    ...
    def update(
        ...
        if events:
            for event in events:
                ...
                print(f"  [ActionClassifier] Frame {frame_index}: {event}")
            return events
        return None
    ...
    def process_video_frames(
        ...
        print(f"[ActionClassifier] Processing {total} frames …")
        for frame_index, frame in enumerate(video_frames):
            if frame_index % 100 == 0:
                print(f"  [ActionClassifier] Frame {frame_index}/{total}")
            detections = self.run_inference(frame)
            events = self.update(frame_index, detections)
            if events:
                shot_events_by_frame[frame_index] = events
        print(
            f"[ActionClassifier] Done. Detected {len(self.events)} shot event(s)."
        )
    ...
    def save_events(self, path: Path) -> None:
        ...
        print(f"[ActionClassifier] Saved {len(self.events)} events → {path}")
