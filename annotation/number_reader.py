"""
annotation/number_reader.py — Jersey number recognition using EasyOCR.
...
from settings.annotation_config import (
    NUMBER_CLASS_ID,
    PLAYER_CLASS_ID,
    NUMBER_OCR_INTERVAL,
...
def compute_ios_matrix(
    ...
    smaller = np.maximum(smaller, 1e-6)
...
class NumberAnnotator:
    """
    Reads jersey numbers in frames and matches them to tracked players.

    Uses:
      - RF-DETR 'number' class detections (from shared ActionClassifier inference)
    ...
    def __init__(self):
        ...
        print("[NumberAnnotator] Loading EasyOCR reader (digits only, CPU)...")
        ...
        print("[NumberAnnotator] EasyOCR ready.")
    ...
    def _recognize_crops(
        ...
        for xyxy in padded_boxes:
            ...
            try:
                ...
                print(f"  [NumberAnnotator] EasyOCR failed on crop: {e}")
                numbers.append("")
    ...
    def update(
        ...
        Args:
            ...
            player_tracks_for_frame: Dict of {track_id: {"bbox": [x1,y1,x2,y2]}}
                                     from the PlayerDetector for this frame.
            detections: Full sv.Detections from the RF-DETR model inference
                        (shared with ActionClassifier).
        """
