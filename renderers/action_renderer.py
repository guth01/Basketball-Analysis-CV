"""
renderers/action_renderer.py — Draws MADE / MISSED shot-event overlays on video frames.
...
class ActionRenderer:
    ...
    Usage in main pipeline::
        ...
        renderer = ActionRenderer(fps=30)
        output_frames = renderer.draw(output_video_frames, shot_events_by_frame)
    """
    ...
    def draw(
        ...
        Args:
            video_frames:         List of BGR frames (already drawn by other renderers).
            shot_events_by_frame: Mapping of {frame_index: [event, …]} returned by
                                  ``ActionClassifier.process_video_frames()``.
